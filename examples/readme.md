...
既知の M5 ギャップ（Table は v1）: 詳細ビューにはインプレースリネームが無く（Rename はリストビューに切り替えてから実行）、DnD も無い（DnD はリストビューのみ。Table への DnD はヘッダーオフセットが内部なので y→行の公開アクセサが要る）。どちらも framework#9 にフィードバック。
