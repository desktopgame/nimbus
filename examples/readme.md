...
## app_filer
ドッグフーディングの実用アプリ第1号: ファイラー。現在は M1（1ペインのディレクトリ閲覧）。
ディレクトリの中身をアイコン付きの List で表示し（フォルダ先行・大文字小文字無視ソート）、ダブルクリック / Enter でフォルダに入る。ツールバーの↑ボタンまたは Backspace で親ディレクトリへ。上部にカレントパス、下部に件数などのステータスを表示する。起動時はカレントディレクトリから始まる。
List の行アクティベーション（`addActionListener`）と Label のアイコン表示の検証シーンも兼ねる。
