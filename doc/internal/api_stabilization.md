...
別件（小）: TextField に変更リスナーが無く `widget_textfield` がタイマー polling していた。SelectionModel とは無関係の単純な実欠落。**実装済み (2026-06-04)**: `TextField.addChangeListener`/`removeChangeListener`（kind=.change、内容変更時のみ発火・カーソル移動や選択では発火しない）を追加し、`widget_textfield` の polling を撤去。C ABI にも `nmTextFieldOnChange`/`OffChange` を公開。**SelectionModel 本体（下記）は未着手。**
...
**裁定**: 穴ではなく**意図した設計**（少ないプリミティブで合成できている方が良い）。修正は**上の便利層のみ**（`setFixedSize` = min==max の糖衣、`padded` ヘルパ等）。**`preferredSize` / `Insets` をプリミティブとして足すのは却下**（Insets はコンテナ合成で代替、唯一の代償はツリー肥大 = 性能の話）。詳細 framework_backlog #19。**冗長が痛くなってから足す**（先回りしない）。
...
**実装済み (2026-06-04)**: `Dialog.destroy` / `ButtonGroup.destroy` を追加。`widget_dialog` / `widget_radio` を 1 呼び出しに更新。C ABI の `nmDialogDestroy` も `Dialog.destroy` 経由に統一（teardown のドリフト防止）。`deinit` はスタック確保（テスト等）向けに公開のまま残す。
...
**実装済み**: `widget_menu` は既に property パターン（`var ctx_panel` スタック変数 + `putProperty`/`getTyped`）に書き換え済み。examples 全体に file-scope の可変グローバルは無い。
...
## 直近のアクション候補
完了 (2026-06-04 までに): #1 型付きコールバック / #3 別件 TextField 変更リスナー / #6 単一 `destroy()` / #7 グローバル撤去。

残り:
* **SelectionModel の設計**（#3 本体、大きめ。別ドキュメント化の候補）。← 次の一手。List / ComboBox / 将来の Table / Tree の選択基盤。
* layout helper（#5）は framework_backlog #19 に移行・据え置き（冗長が痛くなってから）。
* #2 accept、#4 park。
