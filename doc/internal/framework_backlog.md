...
- 値の命名: 役割別トークン（`surface` / `accent` / `text` / `text_disabled` / `border` …）を
  基本に必要箇所のみウィジェット別、のハイブリッドか（推奨。現状の直書き色は accent 系 /
  surface 数段階 / text 2 種 / border 2 種に大半が畳める見込み）、ウィジェット別の全列挙か。
- 引き方: `Application` が 1 個所有。paint 時の取得経路（root プロパティの lazy walk か、
  `Graphics` 経由で渡すか等）は実装時にコストを見て決める。
- 切り替え単位: アプリ全体のみ（ウィンドウ単位テーマは実需待ち）。切り替え＝全ウィンドウ repaint。
