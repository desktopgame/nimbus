...
## #16 手書き関数の IR を検査下に置く（preamble↔IR の同期穴）
- 状態: 未着手
- 優先度: 中
- 影響範囲: 生成器 `main.zig` または新テスト、`preamble.zig` / `preamble_ir.txt`
- 更新日: 2026-06-02
- 依存: なし

### 何
生成関数はシムが実メソッドを呼ぶのでドリフト不能だが、**手書き関数の IR（`preamble_ir.txt`）は
`preamble.zig` の実 export と人手で一致させているだけで無検査**。シグネチャや存在のズレが黙って残りうる
（この仕組みで唯一の“サイレント故障源”）。これを検査下に入れる。

### 候補アプローチ
- 案A: 手書き関数の IR を、コンパイル検査される小さな Zig 記述子から生成する（export と同じ事実から IR を
  導けば不一致が起きない）。メリット: 真にドリフト不能。デメリット: 手書き preamble の構造を変える必要。
- 案B: テストを足す — `preamble_ir.txt` の各 `"c"` シンボルが実際に export されているか（最低限「存在」）を
  確認。メリット: 既存構造を触らず軽い。デメリット: 「存在」は見るが型の一致までは見ない。
- 判断軸: 完全性（A）vs 低コスト（B）。
- 推奨: まず案B（存在チェック）で穴の大半を塞ぎ、必要なら案A。最終判断は作者。

### 完了条件
手書き関数の IR と実 export の不一致が機械的に検出される。
...
- 状態: 未着手
- 優先度: 低
- 影響範囲: 新ツール/テスト（`framework/src` の `pub fn` 走査）
- 更新日: 2026-06-02
- 依存: なし

### 何
「`nimbus.api` にも preamble にも出ていない公開 `pub fn`」を列挙する lint。現状は A/B/C 監査を手でやって
おり、新メソッド追加→公開忘れ（カバレッジ drift）が不可視。これを機械化する。

### 候補アプローチ
- `framework/src` の公開 `pub fn`（内部 vtable 実装 / create・init・deinit 等は除外）と、生成済み＋手書き
  公開分を突き合わせ、未公開を出力。意図的な非公開（min/max size 等）は許可リストで除外。

### 完了条件
未公開の公開メソッド一覧が `zig build` の一ステップ等で出る（公開漏れが見える）。

---

## #18 生成器の型種をテーブル駆動にリファクタ
- 状態: 未着手
- 優先度: 低
- 影響範囲: 生成器 `main.zig`
- 更新日: 2026-06-02
...
### 何
引数/戻りの型種を 1 つ追加すると、ArgType/Ret union → parse → header → zig shim → call 式 → IR の
**6 箇所を散弾的に編集**する必要があり、アームの付け忘れが起きやすい。種ごとに（C 型 / Zig 型 / call 式 /
IR 表現）を 1 つの記述子へまとめ、各 emit 段がそれを引く形にして散弾を減らす。

### なぜ（保留理由）
現状の方式で動いており、純粋な内部リファクタ（外部挙動は不変）。優先度低。
