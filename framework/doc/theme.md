...
したがって、**テーマを設定しても、テーマを読まない LAF（カスタム paint）の見た目は
変わらない。これは仕様**である。参照は全コンポーネントが常に持つが、それを使うかは
描画コードの自由。1 ウィンドウ内に既定 LAF のウィジェットとカスタム paint が同居する
場合、前者だけがテーマに従う（部分適用も正しい挙動）。
