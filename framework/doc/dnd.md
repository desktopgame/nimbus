...
複数コンポーネントの上に浮くため司令塔の責務。 ゴーストは**入力を取らない純粋な描画**なので、 司令塔が **Window paint の最終パス**として全 overlay の上に重ねて描くのが最も単純。

* `beginDrag` で掴んだ位置のオフセット (カーソル − ドラッグ元原点) を覚え、 各 `updateDrag` で `カーソル − オフセット` にゴーストを描く。
* 描画パスなのでコンポーネントツリーに居らず、 `updateDrag` のヒットテストや capture / フォーカスと一切干渉しない。 `finishDrag` / `cancelDrag` で描画をやめるだけ。
* v1 のゴーストは既定の汎用表示 (半透明の矩形 + アイテムのラベル等)。

ゴーストを overlay コンポーネントとして実装する手もある (カスタムゴーストを Component で差せる) が、 その場合は**ヒットテストから除外**しないと `updateDrag` が毎回ゴーストに当たって受け側を解決できなくなる。 v1 は描画パス方式を採る。 カスタムゴースト (ドラッグ元のスナップショットを焼く / `DragSource` に paint フックを足す) は機能要望。
