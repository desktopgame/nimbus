...
pub const FileChooser = struct {
    ...
    fn buildUi(self: *FileChooser) !void {
        ...
        look_in_label.component.setAlignY(.center);
        self.look_in_combo = try app.comboBox(&.{"."});
        self.look_in_combo.component.setGrowX(1);
        self.look_in_combo.component.setAlignY(.center);
        const icon_size = Component.Size{ .width = 16, .height = 16 };
        const up_button = try app.button("");
        up_button.setIcon(try app.icon(.folder_up));
        up_button.setIconSize(icon_size);
        try up_button.setA11yName("Up");
        up_button.component.setAlignY(.center);
        const home_button = try app.button("");
        home_button.setIcon(try app.icon(.house));
        home_button.setIconSize(icon_size);
        try home_button.setA11yName("Home");
        home_button.component.setAlignY(.center);
        const details_button = try app.button("");
        details_button.setIcon(try app.icon(.table));
        details_button.setIconSize(icon_size);
        try details_button.setA11yName("Details");
        details_button.component.setAlignY(.center);
        const list_button = try app.button("");
        list_button.setIcon(try app.icon(.list));
        list_button.setIconSize(icon_size);
        try list_button.setA11yName("List");
        list_button.component.setAlignY(.center);
