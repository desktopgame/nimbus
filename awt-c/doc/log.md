...
`cb` に `NULL` を渡すとデフォルトの挙動に戻る。

コールバックは同期的に呼ばれ、ログを発生させたスレッドと同じスレッドから実行される。
nimbus は単一 UI スレッド前提で動くため、通常はメインスレッドからのみ呼ばれる。

## デフォルトの挙動
コールバック未設定時は、メッセージを `stderr` に `[LEVEL] [category] message\n` 形式で出力する。
これにより、開発初期で何も設定しなくても重要なメッセージが見える。

## ログレベルの想定用途
- `nmLogLevelDebug`: 開発時の詳細追跡（リソース生成ログ等）。本番では callback 側でフィルタ可能
...
利用者は callback を設定することでエラーの原因を取得できる。
