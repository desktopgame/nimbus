...
pub fn main(init: std.process.Init) anyerror!void {
    ...
    props_lines: while (props_reader.takeDelimiterInclusive('\n')) |took| {
    ...
    while (cp_reader.takeDelimiterInclusive('\n')) |took| {
        ...
        try codepoint_mapping.putNoClobber(allocator, codepoint, mapping_buf);
    } else |err| switch (err) {
        error.EndOfStream => {},
        else => {
            return err;
    ...
    var changes_when_casefolded_exceptions = std.array_list.Managed(u21).init(allocator);
    ...
    defer unique_offsets.deinit(allocator);
    ...
    // First pass
    {
        ...
        while (it.next()) |entry| {
            ...
            if (mappings.len == 1) {
                ...
                const result = try unique_offsets.getOrPut(allocator, offset);
        ...
        try unique_offsets.put(allocator, 0, 0x10FFFF);
        const C = struct {
            vals: []u32,
    ...
    defer mappings_to_index.deinit(allocator);
    ...
    // Second pass
    {
        ...
        while (it.next()) |entry| {
            ...
            const mappings = std.mem.sliceTo(entry.value_ptr, 0);
            if (mappings.len > 1) {
                const result = try mappings_to_index.getOrPut(allocator, entry.value_ptr.*);
    ...
    // Build the stage1/stage2/stage3 arrays and output them
    {
        ...
        defer stage2_blocks.deinit(allocator);

        const empty_block: Block = [_]u8{0} ** 256;
        try stage2_blocks.put(allocator, empty_block, {});
        ...
        while (codepoint <= 0x10FFFF) {
            ...
            codepoint += 1;
            if (codepoint % 256 == 0) {
                const result = try stage2_blocks.getOrPut(allocator, block);
        ...
        try writer.interface.print(
            ...
            \\
            \\pub const cutoff: u21 = {};
            \\pub const cwcf_exceptions_min: u21 = {};
            \\pub const cwcf_exceptions_max: u21 = {};
            \\pub const cwcf_exceptions: [{}]u21 = .{{
        , .{ codepoint_cutoff, std.mem.min(u21, changes_when_casefolded_exceptions.items), std.mem.max(u21, changes_when_casefolded_exceptions.items), changes_when_casefolded_exceptions.items.len });
        for (changes_when_casefolded_exceptions.items) |cp| try writer.interface.print("{}, ", .{cp});

        try writer.interface.print(
            \\
            \\}};
            \\
            \\pub const multiple_start: u21 = {};
            \\pub const stage1: [{}]u8 = .{{
        , .{ multiple_codepoint_start, meaningful_stage1.len });
        for (meaningful_stage1) |entry| try writer.interface.print("{}, ", .{entry});
        ...
        try writer.interface.print(
            ...
            \\pub const stage2: [{}]u8 = .{{
        , .{stage2.len});
        for (stage2) |entry| try writer.interface.print("{}, ", .{entry});

        try writer.interface.print(
            \\
            \\}};
            \\
            \\pub const stage3: [{}]i24 = .{{
        , .{stage3.len});
        for (stage3) |entry| try writer.interface.print("{}, ", .{entry});
