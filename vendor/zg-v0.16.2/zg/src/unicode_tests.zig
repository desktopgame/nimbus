    ...
    while (line_iter.next()) |line| {
        ...
        while (fields.next()) |field| : (field_index += 1) {
            if (field_index == 0) {
                ...
                var got = try Normalize.nfcExact(allocator, input);
...
test "Unicode nfc (stream safe) tests" {
    var arena = heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
...
test "Unicode caseless match tests" {
    var arena = heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();

    var canon_matcher = CaselessMatch.CanonCaselessMatcher.default;
    var compat_matcher = CaselessMatch.CompatibilityCaselessMatcher.default;
    ...
    while (line_iter.next()) |line| {
        var fields = mem.splitScalar(u8, line, ';');
        var decoded: [5][]const u8 = undefined;
        var field_index: usize = 0;

        while (field_index < decoded.len) : (field_index += 1) {
            const field = fields.next() orelse break;
            decoded[field_index] = try decodeNormalizationField(allocator, field);
        }

        try testing.expectEqual(@as(usize, decoded.len), field_index);

        try testing.expect(canon_matcher.match(decoded[0], decoded[1]));
        try testing.expect(canon_matcher.match(decoded[0], decoded[2]));
        try testing.expect(canon_matcher.match(decoded[1], decoded[2]));

        for (decoded, 0..) |a, i| {
            for (decoded[i + 1 ..]) |b| {
                try testing.expect(compat_matcher.match(a, b));
            }
        }
    } else |err| switch (err) {
        error.EndOfStream => {},
...
test "Segmentation GraphemeIterator" {
    ...
    while (line_iter.next()) |raw| {
        ...
        {
            var iter = Graphemes.iterator(this_str);
            ...
            for (want.items, 1..) |want_gc, idx| {
                ...
                for (got_gc.offset..got_gc.offset + got_gc.len) |i| {
                    const this_gc = Graphemes.graphemeAtIndex(this_str, i);
                ...
                var after_iter = Graphemes.iterateAfterGrapheme(this_str, got_gc);
        ...
        {
            var iter = Graphemes.reverseIterator(this_str);
            ...
            while (i > 0) {
                ...
                var before_iter = Graphemes.iterateBeforeGrapheme(this_str, got_gc);
...
test "Normalize.nfc reuses already-normalized input" {
    const input = "déjà vu";

    var got = try Normalize.nfcExact(testing.allocator, input);
    defer got.deinit(testing.allocator);

    try testing.expectEqualStrings(input, got.slice);
    try testing.expect(!got.allocated);
    try testing.expectEqual(@intFromPtr(input.ptr), @intFromPtr(got.slice.ptr));
}

test "Normalize.nfc reuses already-normalized input with ASCII prefix" {
...
fn decodeNormalizationField(allocator: Allocator, field: []const u8) ![]const u8 {
    var cp_buf: [4]u8 = undefined;
    var bytes = std.array_list.Managed(u8).init(allocator);
    defer bytes.deinit();

    var code_points = mem.splitScalar(u8, field, ' ');
    while (code_points.next()) |code_point| {
...
const IterRead = struct {
    ...
    pub fn next(iter: *IterRead) anyerror![]const u8 {
        ...
        const this_line = std.mem.trimEnd(u8, took, "\n");
